...
namespace StreamDeckSharp.Extensions
{
    public static class StreamDeckFullScreenDrawingExtension
    {
        ...
        public static byte[] GetFullScreenBitmap(Bitmap b)
        {
            byte[] imgData = null;
        ...
        public static byte[] GetSmallBitmap(Bitmap b)
        {
            byte[] imgData = null;
            using (var resizedImage = ResizeToKeyStreamDeckImage(b))
            {
                imgData = GetRgbArray(resizedImage);
            }
            return imgData;
        ...
        public static Bitmap ResizeToFullStreamDeckKeyImage(Bitmap b)
        {
            var newBm = new Bitmap(72, 72, PixelFormat.Format24bppRgb);
            double scale = Math.Min((double)72 / b.Width, (double)72 / b.Height);
            ...
            using (var g = Graphics.FromImage(newBm))
            {
                ...
                g.FillRectangle(black, new RectangleF(0, 0, 72, 72));
                g.DrawImage(b, new Rectangle(((int)72 - scaleWidth) / 2, ((int)72 - scaleHeight) / 2, scaleWidth, scaleHeight));
            }
        ...
        public static Bitmap ResizeToKeyStreamDeckImage(Bitmap b)
        {
            var newBm = new Bitmap(buttonPxSize, buttonPxSize, PixelFormat.Format24bppRgb);
            double scale = Math.Min((double)buttonPxSize / b.Width, (double)buttonPxSize / b.Height);
            ...
            using (var g = Graphics.FromImage(newBm))
            {
                ...
                g.FillRectangle(black, new RectangleF(0, 0, fullPxWidth, fullPxHeight));
                g.DrawImage(b, new Rectangle(((int)buttonPxSize - scaleWidth) / 2, ((int)buttonPxSize - scaleHeight) / 2, scaleWidth, scaleHeight));
            }

            return newBm;
        }
        public static byte[] GetSmallRgbArray(Bitmap b)
        {
            ...
            try
            {
                var data = new byte[b.Width * b.Height * 3];
        ...
        public static byte[] GetRgbArray(Bitmap b)
        {
            var rect = new Rectangle(0, 0, b.Width, b.Height);
            var lockData = b.LockBits(rect, ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
